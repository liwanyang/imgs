...
const uploadToIpfsBySource = async source => {
  // @ts-ignore
  const client = ipfsHttpClient('https://ipfs.infura.io:5001/api/v0');
  const { cid } = await client.add(source).catch();
  return cid;
};
const uploadToIpfs = async (file: any, metadata: any) => {
  try {
    // @ts-ignore
    const client = ipfsHttpClient('https://ipfs.infura.io:5001/api/v0');
    ...
    const mediaUrl = `https://ipfs.infura.io/ipfs/${added.path}`;


    const tokenIfps = `22uQKg590xyMoyXnw009LVWFRj8:7714ff958e96d972c413168ce7dd5fb2`;
    const authIFPS = Buffer.from(tokenIfps).toString('base64');
    await fetch(`https://ipfs.infura.io:5001/api/v0/pin/add?arg=${added.path}`, {
      headers: {
...
export const mintNFT = async (
  ...
  const link = await uploadToIpfs(files[0], metadataContent)
